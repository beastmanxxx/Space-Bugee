...
const AddCategoryModal = ({ onClose, onSaveSuccess }) => {
    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-container" style={{ maxWidth: '400px' }} onClick={e => e.stopPropagation()}>
                <div className="modal-header">
                    <h2 style={{ fontSize: '20px' }}>Add New Category</h2>
                    <button onClick={onClose} style={{ fontSize: '24px', color: '#666', border: 'none', background: 'none' }}><i className="ph ph-x"></i></button>
                </div>
                <div className="modal-content">
...
const CategoryProductsModal = ({ category, products, onClose, onSaveSuccess }) => {
    ...
    React.useEffect(() => {
        if (category && products) {
            const initial = products.filter(p => p.category === category.name).map(p => p.id);
    ...
    const handleSave = async () => {
        ...
        products.forEach(p => {
            ...
            const isIn = p.category === category.name;
            if (shouldBeIn && !isIn) {
                batch.update(db.collection('products').doc(p.id), { category: category.name });
            } else if (!shouldBeIn && isIn) {
                batch.update(db.collection('products').doc(p.id), { category: firebase.firestore.FieldValue.delete() });
            }
        });
        try {
            await batch.commit();
    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-container" style={{ maxWidth: '600px', display: 'flex', flexDirection: 'column', maxHeight: '90vh' }} onClick={e => e.stopPropagation()}>
                <div className="modal-header">
                ...
                <div className="modal-content" style={{ overflowY: 'auto', flex: 1, paddingRight: '10px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        {filtered.map(p => (
                            <div key={p.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px', border: '1px solid #eee', borderRadius: '6px', background: selectedIds.includes(p.id) ? '#f8fdf9' : 'white', borderColor: selectedIds.includes(p.id) ? '#c2e7c9' : '#eee' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                    <img src={p.images[0]} style={{ width: '44px', height: '44px', borderRadius: '4px', objectFit: 'cover' }} />
                                    <div>
                                        <div style={{ fontWeight: '500', fontSize: '14px', color: '#1a1a1a' }}>{p.name}</div>
                                        <div style={{ fontSize: '12px', color: '#666' }}>₹{p.price}</div>
                                    </div>
                                </div>
                                <input 
                                    type="checkbox" 
                                    style={{ width: '20px', height: '20px', cursor: 'pointer', accentColor: '#1e8e3e' }}
...
const AdminApp = () => {
    ...
    return (
        <div className="admin-layout">
                            ...
                            <tbody>
                                {categoriesList.map(c => (
                                    <tr key={c.id}>
                                        ...
                                        <td>{c.name}</td>
                                        <td>{productsList.filter(p => p.category === c.name).length}</td>
                                        <td>
            ...
            {isCategoryModalOpen && <AddCategoryModal onClose={() => setIsCategoryModalOpen(false)} onSaveSuccess={fetchCategories} />}
